...
function animateParticules(e, t) {
  ...
  anime.timeline().add({
    targets: n, x: function (e) {
      ...
      return e.endPos.y
    }, radius: .1, duration: anime.random(1200, 1800), easing: "easeOutExpo", update: render_animeParticule
  }).add({
    targets: a,
...
if (canvasEl) {
  var ctx = canvasEl.getContext("2d"), numberOfParticules = 30, pointerX = 0, pointerY = 0, tap = "mousedown",
    ...
    }, 500), render_anime = anime({
      duration: 1 / 0, update: function () {
        ctx.clearRect(0, 0, canvasEl.width, canvasEl.height)
      }
    });
  document.addEventListener(tap, function (e) {
    "sidebar" !== e.target.id && "toggle-sidebar" !== e.target.id && "A" !== e.target.nodeName && "IMG" !== e.target.nodeName && (render_anime.play(), updateCoords(e), animateParticules(pointerX, pointerY))
